...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function setActiveById(id) {
        if (!id) return;
        document.querySelectorAll('.nav-buttons').forEach(n => n.classList.remove('active'));
        // if id is 'hero' treat as Home (root href)
        if (id === 'hero' || id === 'home') {
            const homeSelectors = [
                ".nav-buttons[href='#home']",
                ".nav-buttons[href='/']",
                ".nav-buttons[href='/index.html']",
                ".nav-buttons[href='./index.html']",
                ".nav-buttons[href='index.html']"
            ];
            const all = document.querySelectorAll(homeSelectors.join(','));
            if (all && all.length) { all.forEach(el => el.classList.add('active')); return; }
            const first = document.querySelector('.nav-buttons'); if (first) first.classList.add('active');
            return;
        }
        document.querySelectorAll(".nav-buttons[href*='#" + id + "']").forEach(n => n.classList.add('active'));
    }
